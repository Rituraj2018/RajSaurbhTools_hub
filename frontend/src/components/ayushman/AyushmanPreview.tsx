...
import {
  ...
  generateAyushmanA4SheetCanvas,
  generateAyushmanPDF,
  generateAyushmanCardPDF,
  downloadCanvasImage,
...
export const AyushmanPreview: React.FC<AyushmanPreviewProps> = ({
  cards,
  options,
}) => {
  ...
  useEffect(() => {
    ...
    try {
      const canvas = generateAyushmanA4SheetCanvas(cards, options);
    ...
    try {
      ...
      if (doc) {
        ...
        doc.write(`
          ...
          <html>
            <head>
              <title>Ayushman Card Print Sheet (A4)</title>
              <style>
                @page {
                  size: A4 portrait;
                  margin: 0;
            ...
            <body>
              <img id="print-sheet-img" src="${dataUrl}" alt="Ayushman Card A4 Sheet" />
            </body>
          </html>
        `);
        doc.close();

        const printImg = doc.getElementById('print-sheet-img') as HTMLImageElement | null;
        const triggerPrint = () => {
          try {
  ...
  const currentDoc = cards[selectedCardIdx] || cards[0];
  ...
  const handlePrintActualSize = (side: 'front' | 'back') => {
    ...
    if (doc) {
      ...
      doc.write(`
        ...
        <html>
          <head>
            <title>Print Ayushman ${label} Card — CR80 Actual Size</title>
            <style>
              /*
          ...
          <body>
            <img id="print-card-img" src="${dataUrl}" alt="Ayushman ${label} Card" />
          </body>
        </html>
      `);
      doc.close();

      const printImg = doc.getElementById('print-card-img') as HTMLImageElement | null;
      const triggerPrint = () => {
        try {
          iframe.contentWindow?.focus();
          iframe.contentWindow?.print();
    ...
    <div className="p-5 sm:p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6">
      ...
      {printSizeMode === 'a4' && (
        <>
          ...
          {/* Action CTA Buttons: Download & Print */}
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                <span>{isGenerating ? 'Generating PDF...' : 'Download A4 Sheet'}</span>
              </Button>

              <Button
                variant="outline"
                size="lg"
                className="w-full justify-center border-emerald-500/50 text-emerald-300 hover:border-emerald-400 hover:text-emerald-200 hover:bg-emerald-500/10 shadow-lg shadow-emerald-500/10"
                disabled={isPrinting || !sheetCanvas}
                onClick={handlePrint}
                leftIcon={
                  isPrinting ? (
                    <RefreshCw className="w-4 h-4 animate-spin" />
              ...
              >
                <span>{isPrinting ? 'Preparing Print...' : 'Print A4 Sheet'}</span>
              </Button>
            </div>
              ...
              onGetFile={async () => {
                ...
                return new Promise((resolve) => {
                  sheetCanvas.toBlob((blob) => {
                    ...
                    resolve({
                      blob,
                      fileName: `Ayushman_Card_A4_Print_Sheet_${Date.now()}.png`,
                      mimeType: 'image/png',
                      category: 'Documents',
                    });
                  }, 'image/png');
            ...
            {sheetCanvas && (
              <p className="text-[10px] text-slate-500 text-center leading-relaxed">
                Print dialog will show only the A4 Ayushman layout — no UI elements.
              </p>
            )}
          </div>
        </>
      ...
      {/* ── ACTUAL CARD SIZE MODE (CR80 85.6 × 54 mm) ────────────────────── */}
      {printSizeMode === 'actual' && (
        <div className="space-y-6">
          {/* Multi-Card Selector (if multiple cards loaded) */}
          {cards.length > 1 && (
            <div className="flex items-center gap-2 p-2 rounded-2xl bg-slate-950/80 border border-slate-800 overflow-x-auto">
              ...
              {cards.map((c, i) => (
                ...
                >
                  Card {i + 1}: {c.name.slice(0, 14)}
                </button>
              ))}
            </div>
          )}

          {/* CR80 Dimensions Info Banner */}
          <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-emerald-400/70">85.6 × 54 mm — Standard Physical Card Dimensions</p>
              </div>
            </div>
          ...
          {/* Front Card Preview & Actions */}
          <div className="space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-300">Front Card</span>
            ...
            >
              Print Front Card (Actual Size 85.6 × 54 mm)
            </Button>
          </div>
          ...
          {/* Back Card Preview & Actions */}
          <div className="space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-300">Back Card</span>
              <span className="text-[10px] font-mono text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                85.6 × 54 mm
              </span>
            </div>
            ...
            <Button
              ...
              leftIcon={<Printer className="w-3.5 h-3.5 text-emerald-400" />}
            >
              Print Back Card (Actual Size 85.6 × 54 mm)
            </Button>
          </div>
          ...
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 space-y-1.5">
            ...
            <ul className="space-y-1 pl-5 list-disc text-[10px] text-amber-400/80 leading-relaxed">
              ...
              <li>
                If your printer supports CR80 / PAN card paper — select it for direct card printing
              </li>
              <li>
                On standard A4 printers — the card prints at actual 85.6 × 54 mm; trim along the card edge
              </li>
            </ul>
          </div>
        </div>
