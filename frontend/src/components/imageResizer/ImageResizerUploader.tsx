...
import { validateImageFile } from '../../utils/imageResizerProcessor';

interface ImageResizerUploaderProps {
  onFileSelected: (file: File) => void;
  isLoading?: boolean;
}

export const ImageResizerUploader: React.FC<ImageResizerUploaderProps> = ({
  onFileSelected,
  isLoading = false,
}) => {
  ...
  const processFile = (file: File) => {
    ...
    if (!validation.valid) {
      setErrorMessage(validation.error || 'Invalid file. Please select a JPG, PNG, or WebP image.');
      return;
    }
    onFileSelected(file);
  };
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">
              Select or Drop Your Image to Resize
            </h3>
            <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto">
              Change image dimensions by exact pixels or percentage with optional aspect ratio lock and live preview.
            </p>
          </div>
