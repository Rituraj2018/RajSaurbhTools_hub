...
import { validateJpgFile } from '../../utils/jpgToPngProcessor';

interface JpgUploaderProps {
  onFileSelected: (file: File) => void;
  isLoading?: boolean;
}

export const JpgUploader: React.FC<JpgUploaderProps> = ({
  onFileSelected,
  isLoading = false,
}) => {
  ...
  const processFile = (file: File) => {
    ...
    const validation = validateJpgFile(file);
    if (!validation.valid) {
      setErrorMessage(validation.error || 'Invalid file. Please select a JPG or JPEG file.');
      return;
    }
    onFileSelected(file);
  };
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">
              Select or Drop Your JPG/JPEG Image
            </h3>
            <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto">
              Convert any standard JPG or JPEG picture into a crisp, lossless PNG graphic with zero server upload.
            </p>
          </div>
