...
import { UploadCloud, AlertCircle, ShieldCheck, FileType } from 'lucide-react';
import { validateWordFile } from '../../utils/wordToPdfProcessor';

interface WordToPdfUploaderProps {
  onFileSelected: (file: File) => void;
  isLoading?: boolean;
}

export const WordToPdfUploader: React.FC<WordToPdfUploaderProps> = ({
  onFileSelected,
  isLoading = false,
}) => {
  ...
  const processFile = (file: File) => {
    ...
    if (!validation.valid) {
      setErrorMessage(validation.error || 'Invalid file. Please select a Word document.');
      return;
    }
    onFileSelected(file);
  };
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">
              Select or Drop Your Word Document
            </h3>
            <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto">
              Convert Word (.docx, .doc) files into clean, professional, standardized PDF documents in your browser.
            </p>
          </div>
